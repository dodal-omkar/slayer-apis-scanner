...
Changelog v4.1:
 - [CRITICAL SECURITY] API key sanitization in PoC output (prevents leakage in screenshots/logs)
 ...
 - [Feature] Added comprehensive Gemini AI testing (generateContent, embedContent, countTokens)
 - [Feature] Added Gemini model listing and file operations  
 - [Feature] Added PaLM 2 text generation endpoint
 - [Feature] Added Speech-to-Text API
 - [Feature] Added Natural Language API (sentiment, entities, syntax)
 - [Improvement] Total reliable endpoints: 35 (removed 4 weak probes from v4.0)
 - [Security] All curl commands now mask API keys
 - [Security] All responses mask API keys
 - [Accuracy] permission_denied now correctly classified as restricted_key, not API-not-enabled
...
def print_vuln(name, url, note=None, method="GET", headers=None, data=None, json_body=None, response_preview=None, apikey=None):
    with PRINT_LOCK:
        print(f"\n{Colors.FAIL}[VULN]{Colors.ENDC} {name}")
        
        if POC_MODE:
            curl_cmd = generate_curl_command(method, url, headers, data, json_body)
            # CRITICAL: Sanitize API key from curl command to prevent leakage
            if apikey:
                curl_cmd = sanitize_key(curl_cmd, apikey)
            print(f"{Colors.GRAY}       PoC cURL: {Colors.ENDC}{curl_cmd}")
            if response_preview:
                # CRITICAL: Sanitize API key from response
                if apikey:
        ...
        if note:
            # CRITICAL: Sanitize API key from notes
            if apikey:
        ...
        VULNERABLE_APIS.append({
            "name": name,
            "url": url,  # URL kept for internal use
            "note": note,
            "curl": sanitized_curl,
...
def classify_reason(reason, status_code):
    ...
    # NOTE: permission_denied is NOT included here - it's too broad and can mean many things
    if (
    ...
    # Restrictions (IP/Referer/Permission)
    # permission_denied goes here - it usually means auth/IAM restrictions
    if (
...
def sanitize_key(text: str, key: str) -> str:
    """
    Sanitize API key from text to prevent leakage in:
    - Screenshots
    - Terminal history
    - PoC commands
    - Log files
...
def check_endpoint(
    ...
    if expect_image:
        if resp.status_code == 200 and is_image_response(resp):
            print_vuln(name, url, f"Returned image ({len(resp.content)} bytes)", 
                      method=method, headers=headers, apikey=apikey_for_header)
            return True
    ...
    if resp.status_code in (200, 201):
        try:
            ...
            if isinstance(j, dict):
                # Check for explicit error object at top level
                if j.get("error"):
                    error_obj = j["error"]
                ...
                # Check for errors in responses array (Vision API pattern)
                if "responses" in j and isinstance(j["responses"], list):
                    for response_item in j["responses"]:
                        if isinstance(response_item, dict) and response_item.get("error"):
                            # Has error in response - not a successful API call
                            return False
                ...
                snippet = str(j).replace('\n', ' ')  # Full output, just remove newlines for Note field
                response_preview = json.dumps(j, indent=2) if POC_MODE else None
                print_vuln(name, url, f"Success: {snippet}", 
                          method=method, headers=headers, 
                          data=data, json_body=json_body, response_preview=response_preview,
                          apikey=apikey_for_header)
                return True
        except ValueError:
...
def scan_key(apikey, run_ai=True, run_fcm=True, project_id=None, threads=MAX_WORKERS):
    ...
    # 21. Generative Language API - List Files
    tasks.append(("Gemini - List Files", "GET", 
                 f"https://generativelanguage.googleapis.com/v1beta/files?key={apikey}", 
                 {"header_fallback": True, "apikey_for_header": apikey}))
    
    # 22. Gemini - List Models
    tasks.append(("Gemini - List Models", "GET", 
                 f"https://generativelanguage.googleapis.com/v1beta/models?key={apikey}", 
                 {"header_fallback": True, "apikey_for_header": apikey}))
    
    # 23. Gemini Pro - Generate Content (CRITICAL - AI model access)
    if run_ai:
        gemini_body = {
            "contents": [{
                "parts": [{"text": "Say hello from slayer scanner"}]
            }]
        }
        tasks.append(("Gemini Pro - generateContent", "POST", 
                     f"https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key={apikey}", 
                     {"json_body": gemini_body, "header_fallback": True, "apikey_for_header": apikey}))
        
        # 24. Gemini Pro Vision - Generate Content with image
        gemini_vision_body = {
            "contents": [{
                "parts": [
                    {"text": "Describe this image"},
                    {
                        "inline_data": {
                            ...
                            "data": "/9j/4AAQSkZJRg=="  # Tiny 1x1 JPEG
                        }
                    }
                ]
            }]
        }
        tasks.append(("Gemini Pro Vision - generateContent", "POST", 
                     f"https://generativelanguage.googleapis.com/v1beta/models/gemini-pro-vision:generateContent?key={apikey}", 
                     {"json_body": gemini_vision_body, "header_fallback": True, "apikey_for_header": apikey}))
        
        # 25. Gemini - Embed Content
        embed_body = {
            "model": "models/embedding-001",
            "content": {"parts": [{"text": "test embedding"}]}
        }
        tasks.append(("Gemini - embedContent", "POST", 
                     f"https://generativelanguage.googleapis.com/v1beta/models/embedding-001:embedContent?key={apikey}", 
                     {"json_body": embed_body, "header_fallback": True, "apikey_for_header": apikey}))
        
        # 26. Gemini - Count Tokens
        tokens_body = {
            "contents": [{
                "parts": [{"text": "test token count"}]
            }]
        }
        tasks.append(("Gemini - countTokens", "POST", 
                     f"https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:countTokens?key={apikey}", 
                     {"json_body": tokens_body, "header_fallback": True, "apikey_for_header": apikey}))
        
        # 27. PaLM 2 - Generate Text
        palm_body = {
            "prompt": {"text": "Say hello from slayer scanner"}
        }
        tasks.append(("PaLM 2 - generateText", "POST", 
